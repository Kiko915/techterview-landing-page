<template>
    <div v-if="processing && showProcessing"
        class="flex size-full items-center justify-center text-2xl font-bold text-primary/50">
        <span> Processing Logo </span>
    </div>
    <canvas ref="liquidLogoRef"
        :class="cn('block size-full object-contain', props.class, { hidden: processing })"></canvas>
</template>

<script lang="ts" setup>
import { ref, onMounted, onUnmounted, watch, type HTMLAttributes } from "vue";
import { liquidFragSource, vertexShaderSource } from "./shader.ts";
import { parseLogoImage } from "./parseLogoImage.ts";

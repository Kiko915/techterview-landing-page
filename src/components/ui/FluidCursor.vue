<template>
  ...
  onMounted(() => {
    ...
    function getWebGLContext(canvas: HTMLCanvasElement) {
      type FormatPair = { internalFormat: number; format: number };
      const params = {
        alpha: true,
        depth: false,
        stencil: false,
      ...
      let formatRGBA: FormatPair;
      let formatRG: FormatPair;
      let formatR: FormatPair;
      ...
      if (isWebGL2) {
        ...
        formatRGBA =
          ...
          ({ internalFormat: gl.RGBA, format: gl.RGBA } as FormatPair);
        formatRG =
          getSupportedFormat(gl, gl.RGBA, gl.RGBA, halfFloatTexType) ||
          ({ internalFormat: gl.RGBA, format: gl.RGBA } as FormatPair);
        formatR =
